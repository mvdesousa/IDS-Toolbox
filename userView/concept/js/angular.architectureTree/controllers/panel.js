angular.module('ChartsApp').controller('panelCtrl', function ($scope, $timeout, $window, data, bus) {
    ...
    $scope.addBreakpoint = function() {
        if (typeof ($scope.node.breakpoint) === 'undefined') {
            $scope.node.breakpoint = {};
        }
        $scope.node.breakpoint[''] = [];
        $scope.breakpointKeys[''] = '';
    };

    $scope.deleteBreakpoint = function(key) {
        delete $scope.breakpointKeys[key];
        delete $scope.node.breakpoint[key];
    };
});
